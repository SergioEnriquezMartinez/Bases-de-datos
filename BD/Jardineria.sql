...
/*Listado de productos con el valor maximo, minimo y de media*/

SELECT nombre,
MAX(precio_venta) AS Maximo,
MIN(precio_venta) AS Minimo,
AVG(precio_venta) AS Media
FROM producto
GROUP BY nombre;
...
SELECT nombre, precio_venta
FROM producto
WHERE precio_venta <= (SELECT AVG(precio_venta) * 1.1
                        FROM producto)
AND precio_venta >= (SELECT AVG(precio_venta) * 0.9
                        FROM producto)
;

/*Media del precio en funcion del proveedor*/

SELECT proveedor, AVG(precio_proveedor)
FROM producto
GROUP BY proveedor;

/*La ganancia media por proveedor*/

SELECT proveedor, AVG(precio_venta - precio_proveedor)
FROM producto
GROUP BY proveedor;

/*Queremos saber cuantos proveedores hay*/
...
--El count sirve para contar
--El distinct sive para que no salgan resultados repetidos

/*Cuantos productos me reparte cada proveedor*/
...
/*El producto y el proveedor con el que obtengo mayor beneficio*/

SELECT MAX(precio_venta - precio_proveedor), proveedor, nombre AS producto
FROM producto
;

/*Que proveedores me dan mas de 10 productos*/
